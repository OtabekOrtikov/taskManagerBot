...
async def create_project(callback: types.CallbackQuery, state: FSMContext):
    ...
    if lang == "ru":
        send_message = await callback.message.edit_text("Вы создаете проект.\nВведите название проекта.", reply_markup=InlineKeyboardMarkup(back_to_main[lang]))
    elif lang == "uz":
        send_message = await callback.message.edit_text("Siz loyihani yaratyapsiz.\nLoyiha nomini kiriting.", reply_markup=InlineKeyboardMarkup(back_to_main[lang]))
...
async def creating_project(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as connection:
        await connection.execute("INSERT INTO projects (project_name, boss_id) VALUES ($1, $2)", message.text, user['id'])

    if lang == "en":
        await message.answer(f"Project {message.text} has been successfully created. If you want to add participants, create a task for this project.")
    elif lang == "ru":
        await message.answer(f"Проект {message.text} успешно создан. Если хотите добавить участников, создайте для этого проекта задачу.")
    elif lang == "uz":
        await message.answer(f"{message.text} loyihasi muvaffaqiyatli yaratildi. Agar qatnashchilarni qo'shmoqchi bo'lsangiz, ushbu loyiha uchun vazifa yarating.")
