...
from qarz_utils.basic_texts import creation_getdebt_text, next_page_text, noactive_btn

async def skip_comment_getdebt(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = creation_getdebt_text(user, data)
    ...
    if len(users) > PAGE_SIZE:
        keyboard.append([InlineKeyboardButton(text=next_page_text, callback_data=f"getdebt_userlist_{new_debt_id}_2")])
