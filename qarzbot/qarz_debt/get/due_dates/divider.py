...
async def handle_division_choice(callback: types.CallbackQuery, state: FSMContext):
    ...
    loan_date = datetime.strptime((await state.get_data()).get("getdebt_loan_date"), "%d.%m.%Y")
    total_amount = int((await state.get_data()).get("getdebt_amount"))
    ...
    await state.update_data({"getdebt_due_date": due_dates_json, "getdebt_amounts": divided_amounts_json})
    ...
    currency = (await state.get_data()).get("getdebt_currency")
    ...
    send_message = await callback.message.edit_text(text[lang], reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=skip_text[lang], callback_data="getdebt_skip_comment")],
        [back_btn[lang]]
    ]))
    await state.update_data({"main_message_id": send_message.message_id})
    await state.set_state(GetDebtCreation.getdebt_comment)
