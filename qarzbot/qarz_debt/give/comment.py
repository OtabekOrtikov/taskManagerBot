...
from qarz_utils.basic_texts import creation_givedebt_text, next_page_text

async def process_comment_givedebt(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    user = await get_user(user_id)
    lang = user['lang']

    comment = message.text
    await state.update_data({"givedebt_comment": comment})
