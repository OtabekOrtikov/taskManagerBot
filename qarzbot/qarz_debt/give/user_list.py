...
from qarz_utils.basic_texts import creation_givedebt_text, next_page_text, noactive_btn, previous_page_text

async def givedebt_userlist(callback: types.CallbackQuery, state: FSMContext):
    ...
    if page > 1:
        keyboard.append([InlineKeyboardButton(text=previous_page_text[lang], callback_data=f"givedebt_userlist_{new_debt_id}_{page - 1}")])
    if page < total_page:
        keyboard.append([InlineKeyboardButton(text=next_page_text[lang], callback_data=f"givedebt_userlist_{new_debt_id}_{page + 1}")])    
